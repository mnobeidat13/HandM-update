...
#########################################################################################
#########################################################################################
    if page_selection == "Product Labeling": 
        
        labels = pd.read_csv('product_tagging.csv',  dtype={'article_id':str})
        true = labels[labels.label == labels.prediction]
        false = labels[labels.label != labels.prediction]
        
        get_item = st.sidebar.button('Get Random Items')      
        st.sidebar.warning('Labeling a product given its image')
        
        components.html(f"""
        <header>
        <h3 style="color : #00165a;">
        Predictions are generated by a Random Forest model trained on image features extracted using a VGG16 CNN
          </h3>
               </header>
        """)
        if get_item:
            
            rand_true = list(np.random.choice(true.article_id, 3))
            rand_false = list(np.random.choice(false.article_id, 2))
            
            rand_articles = rand_true+rand_false
            random.shuffle(rand_articles)
            cols = st.columns(5)

            for col, rand_article in zip(cols, rand_articles):
                label = labels[labels.article_id == rand_article].label.iloc[0]
                prediction = labels[labels.article_id == rand_article].prediction.iloc[0]
                col.image(get_item_image(str(rand_article[1:]), resize=True, width=300, height=400))
                color =  '#008000' if label == prediction else '#b30000'
                with col:
...
#########################################################################################
    ...
    if page_selection == "Documentation":
        ...
        <a href="https://www.kaggle.com/code/mohammedobeidat/predict-product-features-from-images", target="_blank">
        https://www.kaggle.com/code/mohammedobeidat/predict-product-features-from-images</a> 
        </li>
